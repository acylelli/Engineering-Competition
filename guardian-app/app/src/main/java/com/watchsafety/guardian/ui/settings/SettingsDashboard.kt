...
fun SettingsScreen(
    ...
    Scaffold(topBar = { GuardianTopBar(title = "설정") }) { innerPadding ->
        LazyColumn(
            ...
            item { BatteryStatusCard(watchStatus = watchStatus) }
            item { SectionTitle("알림 설정") }
            item {
                SettingsCard {
                    NotificationSettingRow("SOS 긴급 알림", settings.sosAlert) {
                        onSettingsChange(settings.copy(sosAlert = it))
                    }
                    HorizontalDivider(color = DividerColor)
                    NotificationSettingRow("안전구역 이탈 알림", settings.safeZoneExitAlert) {
                        onSettingsChange(settings.copy(safeZoneExitAlert = it))
                    }
                    HorizontalDivider(color = DividerColor)
                    NotificationSettingRow("집 도착 알림", settings.arrivalAlert) {
                        onSettingsChange(settings.copy(arrivalAlert = it))
                    }
                    HorizontalDivider(color = DividerColor)
                    NotificationSettingRow("배터리 부족 알림", settings.batteryLowAlert) {
                        onSettingsChange(settings.copy(batteryLowAlert = it))
                    }
                }
            }
            item { SectionTitle("일반") }
            item {
                SettingsCard {
                    MenuSettingRow(
                        title = "안전구역 관리",
    ...
    Card(
        ...
        Row(
            ...
            Box(
                ...
                Card(
                    colors = CardDefaults.cardColors(containerColor = TrustBlueContainer),
                    shape = CircleShape,
                ) {
                    Box(modifier = Modifier.size(50.dp), contentAlignment = Alignment.Center) {
                        Icon(Icons.Outlined.Person, contentDescription = null, tint = TrustBlue)
                    }
                }
            }
            Column(
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Text(
                        text = "${watchStatus.deviceName} ${if (watchStatus.isConnected) "연결됨" else "연결 끊김"}",
                        color = TextSecondary,
                    ...
                    Box(
                        ...
                        colors = CardDefaults.cardColors(
                            containerColor = if (watchStatus.isConnected) SafeGreen else TextSecondary,
                        ),
                            shape = CircleShape,
                ...
                Text(
                    text = "보호자: ${user.guardianName} (${user.guardianRelationship})",
                    color = TextSecondary,
...
@Composable
private fun BatteryStatusCard(watchStatus: WatchStatus) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .then(Modifier),
        ...
        Card(
            ...
            Row(
                ...
                Text(
                    text = "배터리 ${watchStatus.batteryPercent}%",
                    modifier = Modifier
                        .weight(1f)
                        .padding(start = 8.dp),
                    color = SafeGreen,
                ...
                Column {
                    ...
                    Text(
                        text = watchStatus.lastConnectedLabel,
                        color = SafeGreen,
                        fontWeight = FontWeight.Bold,
                    )
                }
...
private fun SettingsDashboardPreview() {
    ...
    WatchSafetyTheme {
        SettingsScreen(
            ...
            settings = preview.notificationSettings,
            onSettingsChange = {},
            onUserStatusClick = {},
            onSafeZonesClick = {},
        )
